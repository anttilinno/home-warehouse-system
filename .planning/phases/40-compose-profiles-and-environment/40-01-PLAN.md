...
Purpose: COMP-01 through COMP-05 — dev profile runs only infra, prod profile runs the full stack with database isolation.
...
4. **Update backend, worker, scheduler** database URLs only (keep existing depends_on unchanged):
   - Do NOT modify `depends_on` — all three KEEP their existing `depends_on: migrate: condition: service_completed_successfully` and `redis: condition: service_healthy`. The migrate service already depends on postgres-prod, so the dependency chain is: postgres-prod → migrate → backend/worker/scheduler.
...
5. **Add `postgres-prod-data` to the volumes section** at the bottom of the file, alongside the existing `photo-storage:` volume (already present at line 230 — do NOT duplicate it, just add `postgres-prod-data:` next to it).

Do NOT change the dev postgres or redis services -- they stay as-is with no profile (always start).
  </action>
  <verify>
Run `grep -c "profiles:" docker-compose.yml` -- should show 10 total (6 existing: migrate, backend, worker, scheduler, frontend, angie + 4 newly added: postgres-prod, docspell-postgres, docspell-restserver, docspell-joex).
...
8. `photo-storage:` volume is present (pre-existing, not duplicated)
9. `postgres-prod-data:` volume is present (newly added)
</verification>
...
- No debug flags, dev credentials properly parameterized, photo storage uses existing named volume (`photo-storage:` already defined)
</success_criteria>
