...
## Open Questions (for planner to RESOLVE — verify against source)
1. **Is byte-storage truly unwired, or partially?** Read attachment/service.go UploadFile fully + whether a serve/download route exists. Confirm the exact Storage injection point in router.go (the photoStorage instance) to reuse.
2. **Is there ANY existing attachment integration test?** Grep backend for attachment _test under //go:build integration. If none, ATT-03's test is net-new.
3. **Paperless link contract** — confirm CreateAttachment accepts external_doc_id + dms_type from the request body (attachment/handler.go CreateAttachment input). If yes, PPL-03 is a frontend-only wire.
4. **Migration needed for byte storage?** Probably NOT (files.storage_key already exists; just write bytes to it). Confirm no schema change needed.
5. **Settings-hub slot** — how Phase-12 settings pages register (SettingsLayout + landing rows + route). Mirror for PaperlessPage.
