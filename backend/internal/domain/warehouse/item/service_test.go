...
func (m *MockRepository) FindByWorkspaceFiltered(ctx context.Context, workspaceID uuid.UUID, filters ListFilters, pagination shared.Pagination) ([]*Item, int, error) {
...
func mockSliceIntErrGuarded[T any](args mock.Arguments) ([]T, int, error) {
	if args.Get(0) == nil {
		return nil, args.Int(1), args.Error(2)
	}
...
func (m *MockRepository) FindNeedingReview(ctx context.Context, workspaceID uuid.UUID, pagination shared.Pagination) ([]*Item, int, error) {
	args := m.Called(ctx, workspaceID, pagination)
	return mockSliceIntErrGuarded[*Item](args)
}
...
func (m *MockCategoryRepository) FindRootCategories(ctx context.Context, workspaceID uuid.UUID) ([]*category.Category, error) {
	args := m.Called(ctx, workspaceID)
	return mockSliceErrGuarded[*category.Category](args)
}
...
func TestItem_SetShortCode(t *testing.T) {
	...
	// NewItem doesn't set shortCode - it's empty until set or auto-generated by service
	assert.Empty(t, item.ShortCode())
	originalUpdatedAt := item.UpdatedAt()
	time.Sleep(time.Millisecond)
...
func TestService_Create(t *testing.T) {
	...
	tests := []struct {
		...
		{
			testName: "empty short code triggers auto-generation",
			input: CreateInput{
				WorkspaceID:   workspaceID,
				SKU:           "SKU-011",
				Name:          "Test Item",
...
func TestService_GetByID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			testName:    "item not found - returns error",
			itemID:      itemID,
			workspaceID: workspaceID,
			setupMock: func(m *MockRepository) {
				m.On("FindByID", ctx, itemID, workspaceID).Return(nil, ErrItemNotFound)
			},
			expectError: true,
		...
		},
		{
			...
			workspaceID: uuid.New(),
			setupMock: func(m *MockRepository) {
				m.On("FindByID", ctx, itemID, mock.Anything).Return(nil, ErrItemNotFound)
			},
			expectError: true,
			errorType:   ErrItemNotFound,
		},
...
func TestService_Update(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(m *MockRepository) {
				m.On("FindByID", ctx, mock.Anything, workspaceID).Return(nil, ErrItemNotFound)
			},
			expectError: true,
			errorType:   ErrItemNotFound,
		},
