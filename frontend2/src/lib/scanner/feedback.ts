 ...
 * - Call resumeAudioContext() inside the opening user-gesture handler on iOS
 *   (Phase 64 D-08) — the AudioContext is created suspended on iOS and must
 *   be resumed from within a user gesture to become audible.
 */
...
/**
 * Resume the module-scope AudioContext if it is suspended.
 *
 * Phase 64 addition (D-08) — iOS Safari creates AudioContexts in the
 * "suspended" state; they must be `resume()`d from within the opening user
 * gesture (e.g. a `pointerdown` handler on the page). This helper is a no-op
 * when the context is already running or when no context has been created.
 *
 * Errors from `resume()` are swallowed: iOS rejects the call when it is not
 * fired inside a gesture, and retrying on the next gesture is the correct UX.
 */
 ...
 *
 * NOTE: Not supported on iOS Safari - fails silently (Phase 64 D-17 LOCKED:
 * no native-haptic dependency; iOS gets audio + visual feedback only).
 *
